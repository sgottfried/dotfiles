...
(defun my/debug-jest-current-buffer ()
  "Debug Jest test in the current buffer using Node debugger."
  (interactive)
  (let* ((project-root (projectile-project-root))
         (file-name (file-relative-name buffer-file-name project-root))
         (debug-command (format "node --inspect-brk ./node_modules/.bin/jest --runInBand --no-cache %s" file-name)))
    (dap-debug
     `((:type "node"
       :request "launch"
       :name "Debug Jest Tests"
       :program "${workspaceFolder}/node_modules/.bin/jest"
       :args ["--runInBand", "--no-cache", ,file-name]
